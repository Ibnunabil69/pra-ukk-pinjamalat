...
@section('content')
    <div class="max-w-xl mx-auto px-4 md:px-0">
        ...
        <!-- HEADER -->
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 mt-4 md:mt-0">
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Edit User</h1>
                <p class="text-gray-500 mt-1">Ubah data user admin, petugas, atau peminjam</p>
            </div>
                ...
                <div class="mb-4">
                    ...
                    <div class="relative">
                        <button type="button"
                            ...
                            id="roleDropdownBtn">
                            <span id="roleDropdownLabel">{{ old('role', ucfirst($user->role)) }}</span>
                        ...
                        <input type="hidden" name="role" id="roleDropdownInput" value="{{ old('role', $user->role) }}">
                    </div>
                    @error('role')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
    ...
    <script>
        ...
        // Reset form (kembali ke data awal user)
        document.getElementById('resetBtn').addEventListener('click', () => {
            document.getElementById('editUserForm').reset();
            document.getElementById('roleDropdownInput').value = '{{ $user->role }}';
            document.getElementById('roleDropdownLabel').textContent = '{{ ucfirst($user->role) }}';
        });
        ...
        confirmModalBtn.addEventListener('click', () => document.getElementById('editUserForm').submit());
    </script>
@endsection
